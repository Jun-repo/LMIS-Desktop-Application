...
class AddNewClientPageState extends State<AddNewClientPage> {
  ...
  final FocusNode _surveyNumberFocus = FocusNode(); // New focus node
  ...
  void _handleLandPropertyPart1Keys(RawKeyEvent event) {
    if (event.logicalKey == LogicalKeyboardKey.arrowDown) {
      if (_bundleNumberFocus.hasFocus)
        ...
        FocusScope.of(context).requestFocus(_surveyNumberFocus); // Updated
      else if (_surveyNumberFocus.hasFocus) // Updated
  ...
  void _handleLandPropertyPart4Keys(RawKeyEvent event) {
    if (event.logicalKey == LogicalKeyboardKey.arrowDown) {
      if (_tplaToPenroTransmittedDateFocus.hasFocus)
      ...
      child: Column(
        ...
        children: [
          ...
          TextFormField(
            ...
            style: const TextStyle(
                color: Colors.white, fontFamily: 'Gilroy', fontSize: 16),
            decoration: customInputDecoration('PLA to PENRO Transmitted Date',
                    hasValue:
        ...
        GestureDetector(
          ...
          child: Container(
            ...
            color: _currentStep == 2
                ? const Color.fromARGB(255, 13, 136, 69)
                : const Color.fromARGB(255, 60, 62, 62),
  ...
  Future<bool> _checkExistingClient() async {
    ...
    if (kDebugMode) {
      print('Checking client: $firstname $middlename $lastname $suffix');
    }

    if (firstname.isEmpty || lastname.isEmpty) {
      if (kDebugMode) {
        print('First name or last name empty, skipping check');
      }
      return false;
    }

    final existingClients = await dbHelper.getClientsByName(
      firstname: firstname,
      lastname: lastname,
    ...
    if (kDebugMode) {
      print('Found ${existingClients.length} existing clients:');
    }
    for (var client in existingClients) {
      if (kDebugMode) {
        print(' - ${client[DatabaseHelper.columnApplicantFirstname]} '
            '${client[DatabaseHelper.columnApplicantMiddlename]} '
            '${client[DatabaseHelper.columnApplicantLastname]} '
            '${client[DatabaseHelper.columnApplicantSuffix]}');
      }
    }

    return existingClients.isNotEmpty;
  }

  void _showDuplicateClientDialog() {
    showDialog(
      context: context,
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: const Text(
            'A client with this first name and last name already exists. Do you want to continue?',
                                    ...
                                    child: ElevatedButton(
                                      onPressed: () async {
                                        if (_formKey.currentState!.validate()) {
                                          setState(() => _isLoading = true);
                                          final exists =
                                              await _checkExistingClient();
                                          setState(() => _isLoading = false);

                                          if (exists) {
                                            _showDuplicateClientDialog();
                                          } else {
                                            setState(() => _currentStep = 1);
                                          }
                                        }
