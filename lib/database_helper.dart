...
class DatabaseHelper {
  ...
  static const _databaseVersion = 3; // Updated from 2 to 3
  ...
  static const columnSurveyNumber = 'survey_number';
  ...
  Future _onUpgrade(Database db, int oldVersion, int newVersion) async {
    ...
    if (oldVersion <= 2 && newVersion >= 3) {
      await db.execute('''
        ALTER TABLE $tableLandProperty 
        ADD COLUMN $columnSurveyNumber TEXT
      ''');
    }
  }
  ...
  Future<List<Map<String, dynamic>>> getClientsByName({
    ...
    final db = await database;
    String whereClause = '''
    UPPER($columnApplicantFirstname) = UPPER(?) AND 
    UPPER($columnApplicantLastname) = UPPER(?)
  ''';
    List<dynamic> whereArgs = [firstname, lastname];

    if (middlename.isNotEmpty) {
      whereClause += ' AND UPPER($columnApplicantMiddlename) = UPPER(?)';
      whereArgs.add(middlename);
    }

    if (suffix.isNotEmpty) {
      whereClause += ' AND UPPER($columnApplicantSuffix) = UPPER(?)';
      whereArgs.add(suffix);
    }
  ...
  Future<List<Map<String, dynamic>>> queryClientsWithLandProperties() async {
    ...
    return await db.rawQuery('''
      SELECT 
        ...
        l.$columnSurveyNumber AS survey_number,
