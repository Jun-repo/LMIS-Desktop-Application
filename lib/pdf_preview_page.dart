...
class _PdfPreviewPageState extends State<PdfPreviewPage> {
  ...
  final List<Map<String, String>> availableFields = [
    ...
    {'key': 'lot_number_survey', 'header': 'Lot & Survey Number'},
    ...
    widget.clientData.forEach((clientId, clientRecord) {
      ...
      if (landProperties.isEmpty) {
        ...
        for (var key in _selectedKeysOrdered) {
          if (key == 'lot_number_survey') {
            row.add(''); // No land properties, so empty for combined field
          } else {
        ...
        for (var property in landProperties) {
          ...
          for (var key in _selectedKeysOrdered) {
            if (key == 'lot_number_survey') {
              // Combine lot_number and survey_number
              final lotNumber = property['lot_number']?.toString() ?? '';
              final surveyNumber = property['survey_number']?.toString() ?? '';
              final combined = surveyNumber.isNotEmpty
                  ? '$lotNumber, $surveyNumber'
                  : lotNumber;
              row.add(combined);
            } else if (client.containsKey(key)) {
              row.add(client[key]?.toString() ?? '');
