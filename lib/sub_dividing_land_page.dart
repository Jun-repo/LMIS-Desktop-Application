...
class SubDividingLandPageState extends State<SubDividingLandPage> {
  ...
  final FocusNode _surveyNumberFocus = FocusNode(); // Added
  ...
  final FocusNode _dateAppliedReceivedFocus = FocusNode(); // Moved up
  ...
  final TextEditingController _surveyNumberController =
  ...
  final TextEditingController _dateAppliedReceivedController =
      TextEditingController(); // Moved up
  ...
  final Map<String, List<String>> _applicationNumbersByMunicipality = {
    'Quezon': ['FPA', 'FLA', 'HPA', 'MSA', 'RPA', 'MLA', 'RFPA', 'SA'],
    'Narra': ['FPA', 'FLA', 'HPA', 'MSA', 'RPA', 'MLA', 'RFPA', 'SA'],
    'Rizal': ['FPA', 'FLA', 'HPA', 'MSA', 'RPA', 'MLA', 'RFPA', 'SA'],
  };
  ...
  void dispose() {
    ...
    _surveyNumberFocus.dispose(); // Added
    ...
    _dateAppliedReceivedFocus.dispose(); // Updated
    ...
    _surveyNumberController.dispose(); // Added
    ...
    _dateAppliedReceivedController.dispose(); // Updated
  ...
  void _handleLandPropertyPart4Keys(RawKeyEvent event) {
    if (event.logicalKey == LogicalKeyboardKey.arrowDown) {
      if (_dateAppliedReceivedFocus.hasFocus)
  ...
  Widget _buildPreviewForm() {
    ...
    return Column(
      ...
      children: [
        ...
        _buildPreviewRow('Application Number', applicationNumber),
        ...
        _buildPreviewRow('Date Applied/Received',
            _dateAppliedReceivedController.text), // Moved up
  ...
  void _editSubdivision(int index) {
    ...
    _surveyNumberController.text = subdivision['land_property']
            [DatabaseHelper.columnSurveyNumber] ??
        ''; // Added
    ...
    if (appNum.trim().isNotEmpty) {
      ...
      if (options != null) {
        String? matchedBase = options
            .firstWhere((item) => appNum.startsWith(item), orElse: () => '');
        if (matchedBase.isNotEmpty) {
          _selectedApplicationNumber = matchedBase;
          _applicationExtensionController.text =
              appNum.substring(matchedBase.length).trim();
        } else {
          _selectedApplicationNumber = null;
  ...
  void _clearForm() {
    ...
    _surveyNumberController.clear(); // Added
    ...
    _dateAppliedReceivedController.clear(); // Updated
