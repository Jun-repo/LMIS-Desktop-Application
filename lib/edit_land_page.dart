...
class EditLandPage extends StatefulWidget {
  ...
  final Function(Map<String, dynamic> updatedLandProperty)
      onLandPropertyUpdated;

  const EditLandPage({
    super.key,
    required this.clientId,
    required this.landPropertyData,
    required this.onLandPropertyUpdated,
  });

  @override
  EditLandPageState createState() => EditLandPageState();
}

class EditLandPageState extends State<EditLandPage> {
  final _formKey = GlobalKey<FormState>();
  int _currentStep = 0;

  // FocusNodes for Part 1
  final FocusNode _lotNumberFocus = FocusNode();
  final FocusNode _surveyNumberFocus = FocusNode(); // New FocusNode
  ...
  late TextEditingController _surveyNumberController; // New Controller
  late TextEditingController _patentNumberController;
  late TextEditingController _identicalNumberController;
  late TextEditingController _bundleNumberController;
  late TextEditingController _areaSqmController;
  late TextEditingController _surveyClaimantController;
  late TextEditingController _sitioController;
  late TextEditingController _dateAppliedReceivedController;
  late TextEditingController _tplaToPenroTransmittedDateController;
  late TextEditingController _returnFromPenroController;
  late TextEditingController _rodReceivedDateController;
  late TextEditingController _dateApprovedController;
  late TextEditingController _serialNumberController;
  late TextEditingController _landInvestigatorController;
  late TextEditingController _applicationExtensionController;
  ...
  void initState() {
    ...
    _lotNumberController = TextEditingController(
        text: widget.landPropertyData[DatabaseHelper.columnLotNumber] ?? '');
    _surveyNumberController = TextEditingController(
        text: widget.landPropertyData[DatabaseHelper.columnSurveyNumber] ??
            ''); // Added
    _patentNumberController = TextEditingController(
        text: widget.landPropertyData[DatabaseHelper.columnPatentNumber] ?? '');
    _identicalNumberController = TextEditingController(
        text: widget.landPropertyData[DatabaseHelper.columnIdenticalNumber] ??
            '');
    _bundleNumberController = TextEditingController(
        text: widget.landPropertyData[DatabaseHelper.columnBundleNumber] ?? '');
    _areaSqmController = TextEditingController(
        text:
            widget.landPropertyData[DatabaseHelper.columnAreaSqm]?.toString() ??
                '');
    _surveyClaimantController = TextEditingController(
        text:
            widget.landPropertyData[DatabaseHelper.columnSurveyClaimant] ?? '');
    _sitioController = TextEditingController(
        text: widget.landPropertyData[DatabaseHelper.columnSitio] ?? '');
    _dateAppliedReceivedController = TextEditingController(
        text:
            widget.landPropertyData[DatabaseHelper.columnDateAppliedReceived] ??
                '');
    _tplaToPenroTransmittedDateController = TextEditingController(
        text: widget.landPropertyData[
                DatabaseHelper.columnTplaToPenroTransmittedDate] ??
            '');
    _returnFromPenroController = TextEditingController(
        text: widget.landPropertyData[DatabaseHelper.columnReturnFromPenro] ??
            '');
    _rodReceivedDateController = TextEditingController(
        text: widget.landPropertyData[DatabaseHelper.columnRodReceivedDate] ??
            '');
    _dateApprovedController = TextEditingController(
        text: widget.landPropertyData[DatabaseHelper.columnDateApproved] ?? '');
    _serialNumberController = TextEditingController(
        text: widget.landPropertyData[DatabaseHelper.columnSerialNumber] ?? '');
    _landInvestigatorController = TextEditingController(
        text: widget.landPropertyData[DatabaseHelper.columnLandInvestigator] ??
            '');
    _applicationExtensionController = TextEditingController();

    // Initialize dropdown values
    _selectedPropertyMunicipality =
        widget.landPropertyData[DatabaseHelper.columnPropertyMunicipality];
    if (_selectedPropertyMunicipality != null &&
        !_municipalities.contains(_selectedPropertyMunicipality))
      _selectedPropertyMunicipality = null;

    _selectedLotStatus =
        widget.landPropertyData[DatabaseHelper.columnLotStatus];
    if (_selectedLotStatus != null &&
        !_lotStatusOptions.contains(_selectedLotStatus))
      _selectedLotStatus = null;

    // Validate Property Barangay
    String? storedBarangay =
        widget.landPropertyData[DatabaseHelper.columnPropertyBarangay];
    if (_selectedPropertyMunicipality != null && storedBarangay != null) {
      List<String>? validBarangays = _barangays[_selectedPropertyMunicipality];
      _selectedPropertyBarangay =
          validBarangays != null && validBarangays.contains(storedBarangay)
              ? storedBarangay
              : null;
    } else {
    ...
    // Split and validate Application Number
    String? storedAppNumber =
        widget.landPropertyData[DatabaseHelper.columnApplicationNumber];
    if (storedAppNumber != null && storedAppNumber.isNotEmpty) {
      if (_selectedPropertyMunicipality != null) {
        List<String>? validAppNumbers =
            _applicationNumbersByMunicipality[_selectedPropertyMunicipality];
        if (validAppNumbers != null) {
          String? matchedBase = validAppNumbers.firstWhere(
            (item) => storedAppNumber.startsWith(item),
            orElse: () => '',
          );
          if (matchedBase.isNotEmpty) {
            _selectedApplicationNumber = matchedBase;
            _applicationExtensionController.text =
                storedAppNumber.substring(matchedBase.length).trim();
          } else {
            _selectedApplicationNumber = null;
            _applicationExtensionController.text = storedAppNumber;
          }
        } else {
          _selectedApplicationNumber = null;
          _applicationExtensionController.text = storedAppNumber;
  ...
  void dispose() {
    ...
    _surveyNumberFocus.dispose(); // Dispose new FocusNode
    ...
    _surveyNumberController.dispose(); // Dispose new Controller
  ...
  void _handleKeyEvent(RawKeyEvent event) {
    ...
    else if (_currentStep == 1) _handlePart2Keys(event);
  }

  void _handlePart1Keys(RawKeyEvent event) {
    if (event.logicalKey == LogicalKeyboardKey.arrowDown) {
      if (_lotNumberFocus.hasFocus)
        FocusScope.of(context).requestFocus(_surveyNumberFocus);
      else if (_surveyNumberFocus.hasFocus)
  ...
  Future<void> _showPreviewDialog() async {
    if (_formKey.currentState!.validate()) {
      String applicationNumber = _selectedApplicationNumber ?? '';
      if (_applicationExtensionController.text.isNotEmpty) {
        applicationNumber =
            '$applicationNumber ${_applicationExtensionController.text}';
      }
            ...
            content: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  ...
                  _buildPreviewItem(
                      'Survey Number', _surveyNumberController.text), // Added
                  _buildPreviewItem(
                  ...
                  _buildPreviewItem('Application Number', applicationNumber),
  ...
  Future<void> _saveLandProperty() async {
    if (_formKey.currentState!.validate()) {
      final dynamic idFromData =
          widget.landPropertyData[DatabaseHelper.columnLandPropertyId] ??
              widget.landPropertyData['land_property_id'];
      if (idFromData == null || idFromData is! int) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Error: Invalid or missing Land Property ID.'),
          ),
        );
        return;
      }
      final int landPropertyId = idFromData;
      ...
      Map<String, dynamic> updatedLandProperty = {
        DatabaseHelper.columnLandPropertyId: landPropertyId,
        DatabaseHelper.columnClientIdRef: widget.clientId,
        DatabaseHelper.columnApplicationNumber: applicationNumber,
        DatabaseHelper.columnLotNumber: _lotNumberController.text,
        DatabaseHelper.columnSurveyNumber:
            _surveyNumberController.text, // Added
      ...
      try {
        int rowsAffected = await DatabaseHelper.instance
            .updateLandProperty(updatedLandProperty);
        if (rowsAffected > 0) {
          widget.onLandPropertyUpdated(updatedLandProperty);
          Navigator.pop(context);
        } else {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text(
                  'Failed to update land property. No changes detected or invalid ID.'),
            ),
          );
        }
      } catch (e) {
            ...
            child: Column(
              children: [
                ...
                TextFormField(
                  ...
                  focusNode: _surveyNumberFocus,
                  decoration: customInputDecoration("Survey Number",
                      hasValue: _surveyNumberController.text.isNotEmpty),
                  style: const TextStyle(
                      color: Colors.white, fontFamily: 'Gilroy', fontSize: 16),
                  onChanged: (_) => setState(() {}),
                ),
                  ...
                  children: [
                            ...
                            .map((e) => DropdownMenuItem(
                                ...
                                child: Text(e,
                                    style: const TextStyle(
                                        ...
                                        fontSize: 16))))
                            .toList(),
                        decoration: customInputDecoration("Application No.",
                    ...
                    Expanded(
                      ...
                      child: TextFormField(
                        ...
                        decoration: customInputDecoration("Extension",
                            hasValue: _applicationExtensionController
                                .text.isNotEmpty),
                        style: const TextStyle(
                            color: Colors.white,
                            fontFamily: 'Gilroy',
