...
class HomePageState extends State<HomePage> with TickerProviderStateMixin {
  ...
  String? extractYear(String? dateStr) {
    if (dateStr == null || dateStr.isEmpty) return null;
    try {
      // Try parsing standard date format (e.g., "1931-04-06")
      final date = DateTime.tryParse(dateStr);
      if (date != null) return date.year.toString();

      // Handle "April 6, 1931" format
      final parts = dateStr.split(' ');
      if (parts.length >= 3) {
        return parts.last; // Last part should be the year
      }
      return null;
    } catch (e) {
      if (kDebugMode) {
  ...
  List<Map<String, dynamic>> get _filteredClients {
    ...
    // Apply ROD filter
    if (_rodFilterStatus == "Patented/Approved") {
      filtered = filtered.where((client) {
        bool hasPatentedApproved =
            (client['land_properties'] as List<dynamic>).any((land) {
          final rodDate =
              land[DatabaseHelper.columnRodReceivedDate]?.toString();
          final lotStatus =
              land[DatabaseHelper.columnLotStatus]?.toString().toLowerCase();
          bool isPatentedApproved = lotStatus == "patented/approved";

          if (kDebugMode) {
            print(
                'Client ID: ${client['client'][DatabaseHelper.columnClientId]}');
          }
          if (kDebugMode) {
            print('  Lot Status: $lotStatus');
          }
          if (kDebugMode) {
            print('  ROD Date: $rodDate');
          }
          if (kDebugMode) {
            print('  Is Patented/Approved: $isPatentedApproved');
          }

          if (_rodFilterYear != null) {
            final year = extractYear(rodDate);
            bool yearMatch = year != null && year == _rodFilterYear;
            if (kDebugMode) {
              print(
                  '  Year Filter: $_rodFilterYear, Extracted Year: $year, Match: $yearMatch');
            }
            return isPatentedApproved && yearMatch;
          }
          return isPatentedApproved;
        });
        if (kDebugMode) {
          print(
              'Client ${client['client'][DatabaseHelper.columnClientId]} has Patented/Approved: $hasPatentedApproved');
        }
        return hasPatentedApproved;
      }).toList();
    ...
    if (kDebugMode) {
      print(
          'Filtering with ROD status: $_rodFilterStatus, Year: $_rodFilterYear');
    }
    if (kDebugMode) {
                      ...
                      child: ElevatedButton.icon(
                        ...
                        onPressed: () {
                          ...
                          final RelativeRect position = RelativeRect.fromLTRB(
                            buttonPosition.dx + margin,
                            buttonPosition.dy + buttonSize.height + margin,
                          ...
                          showMenu(
                            ...
                            items: [
                              ...
                              PopupMenuItem<String>(
                                ...
                                child: const Text("All Patented/Approved",
                                    style: TextStyle(color: Colors.white)),
                                onTap: () {
                                  setState(() {
                                    _rodFilterStatus = "Patented/Approved";
                              ...
                              PopupMenuItem<String>(
                                value: "Patented/Approved by Year",
                                child: const Text("Patented/Approved by Year",
                                    style: TextStyle(color: Colors.white)),
                                onTap: () {
                                                                ...
                                                                Expanded(
                                                                  ...
                                                                  child: Text(
                                                                    [
                                                                      client['client']
                                                                              [
                                                                              DatabaseHelper.columnSitioAddress] ??
                                                                          '',
                                                                      client['client']
                                                                              [
                                                                              DatabaseHelper.columnBarangayAddress] ??
                                                                          '',
                                                                      client['client']
                                                                              [
                                      ...
                                      children: [
                                        Column(
                                          ...
                                          children: [
                                            ...
                                            _buildInfoRow(
                                              'Lot & Survey Number',
                                              '${selectedLandProperty![DatabaseHelper.columnLotNumber] ?? ''}, ${selectedLandProperty![DatabaseHelper.columnSurveyNumber] != null && selectedLandProperty![DatabaseHelper.columnSurveyNumber].isNotEmpty ? ' ${selectedLandProperty![DatabaseHelper.columnSurveyNumber]}' : ''}',
                                            ),
                                            _buildInfoRow(
                                                'Identical Lot Number',
  ...
  Widget _buildTotalClientsLabel() {
    return Padding(
      ...
      child: Align(
        ...
        child: Text('Total Applicants: ${_groupedClients.length}',
            style: const TextStyle(
                color: Colors.white60, fontSize: 12, fontFamily: 'Gilroy')),
      ),
    );
  }
}
